...
function Footer() {
  ...
  return (
    <section className="footer__section bg-[#F3F3F3] py-20">
        ...
        <div className="left-footer-part md:w-1/2 flex flex-col justify-start items-start">
          <button>
            <div className="header-logo flex items-center space-x-2">
              <h2 className="text-3xl text-black font-bold ">Multi</h2>
            </div>
          </button>
          <p className="text-black/80 text-sm mt-5 max-w-xs font-[400]">
        ...
        <div className="right-footer-part pt-5 md:w-1/2">
          <div className="flex gap-x-3 items-center justify-between px-0 lg:px-10">
            {Object.keys(footerLinks[0]).map((columnKey) => (
              <ul
                key={columnKey}
                className="footer-link-list text-black/70 text-sm font-semibold flex flex-col gap-y-2"
              >
                {footerLinks[0][columnKey].map((link) => (
                  <li
                    className="footer-link-item hover:text-black"
                    key={link.id}
                  >
